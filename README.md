# Minnpost 2014 Follow The Money
...
1. Check out this code with [Git](http://git-scm.com/): `git clone https://github.com/MinnPost/minnpost-2014-follow-the-money.git`
...
1. Run: `grunt server`
    * This will run a local webserver for development and you can view the application in your web browser at [http://localhost:8840](http://localhost:8840).
...
1. By default, running a local server will show you the local development version.  But there are other builds that you can view by changing the query parameters.  Do note that you may have to run the build and deploy things for things to work normally.
    * Local build: [localhost:8840/?mpDeployment=build](http://localhost:8840/?mpDeployment=build)
    * Build deployed on S3: [localhost:8840/?mpDeployment=deploy](http://localhost:8840/?mpDeployment=deploy)
    * Embedded version with local build: [localhost:8840/?mpDeployment=build&mpEmbed=true](http://localhost:8840/?mpDeployment=build&mpEmbed=true)
    * Embedded version with S3 build: [localhost:8840/?mpDeployment=deploy&mpEmbed=true](http://localhost:8840/?mpDeployment=deploy&mpEmbed=true)
